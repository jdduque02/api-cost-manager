...
@ApiTags('finance / objective-payments')
@UseGuards(AuthGuard)
@ApiIntrospectGuardResponse()
@Controller('users/:userId/financial-objectives/:objectiveId/payments')
export class ObjectivePaymentController {
  constructor(private readonly objectivePaymentService: ObjectivePaymentService) {}

  @Post()
  @HttpCode(HttpStatus.CREATED)
  @ApiOperation({ summary: 'Registrar abono a un objetivo financiero' })
  @ApiResponse({ status: HttpStatus.CREATED, description: 'Abono registrado.', type: ObjectivePaymentResponseDto })
  @ApiBadRequestResponse({ description: 'Datos de entrada inválidos.', type: ErrorResponseDto })
  @ApiInternalServerErrorResponse({ description: 'Error al registrar el abono.', type: ErrorResponseDto })
  async create(
    ...
    return this.objectivePaymentService.create(userId, { ...dto, objective_id: objectiveId });
  }

  @Get()
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Listar abonos de un objetivo financiero' })
  @ApiResponse({ status: HttpStatus.OK, description: 'Abonos del objetivo.', type: [ObjectivePaymentResponseDto] })
  async findAll(
    @Param('userId', ParseIntPipe) userId: number,
    @Param('objectiveId', ParseIntPipe) objectiveId: number,
    @CurrentUser() _currentUser: IntrospectResponse,
  ...
  @ApiResponse({ status: HttpStatus.OK, description: 'Abono encontrado.', type: ObjectivePaymentResponseDto })
  @ApiNotFoundResponse({ description: 'Abono no encontrado.', type: ErrorResponseDto })
  async findOne(
    @Param('userId', ParseIntPipe) userId: number,
  ...
  @ApiResponse({ status: HttpStatus.NO_CONTENT, description: 'Abono eliminado.' })
  @ApiNotFoundResponse({ description: 'Abono no encontrado.', type: ErrorResponseDto })
  async remove(
    @Param('userId', ParseIntPipe) userId: number,
    @Param('id', ParseIntPipe) id: number,
